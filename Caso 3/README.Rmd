...
La variable labels extrae el nombre de las actividades, esto es útil ya que si hay demasiadas actividades no es forzoso tener que escribir una por una.
...
Usamos la línea del código `encabezados$v2` porque existen dos columnas en "encabezados" y nos interesa solamente la segunda columna.
...
En esta nueva base de datos estarán todos los datos de los 30 participantes que hicieron la prueba y los datos de cada actividad.

###Hacer un libro de texto

Con la última base de datos data lo convertiremos en un libro de texto que aparecerá en el directorio de trabajo utilizamos el paquete `dplyr`.

`prom_data_set <- data %>% group_by(Participante,Actividad) %>% summarise_each(funs(mean))`

La función `write.table` para hacer el libro de texto con la base de datos.
